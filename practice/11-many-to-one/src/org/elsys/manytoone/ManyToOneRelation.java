...
public class ManyToOneRelation<M, O> {
	 ...
	 * @return
	 */
	private List<M> many = new ArrayList<>();
	private Map<List<M>, O> ManyToOneRelation = new HashMap<>();
	
	public boolean connect(M source, O target) {
		many.add(source);
		
		if(ManyToOneRelation.put(many, target) == null) {
			return false;
		}
		return true;
	}
	...
	public boolean containsSource(M source) {
		return ManyToOneRelation.containsKey(source);
	}
	...
	public boolean containsTarget(O target) {
		return ManyToOneRelation.containsValue(target);
	}
	...
	public O getTarget(M source) {
		return ManyToOneRelation.get(source);
	}
