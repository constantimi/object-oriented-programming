...
public class ImageFilters extends JPanel {
    ...
    public void InvertColors() {
        ...
        for(int y = 0; y < image_pixels.getHeight(); y++) {
        	for(int x = 0; x < image_pixels.getWidth();  x++) {
        		Pixel p = getPixel(x,y,image_pixels);
    ...
    public void ConvertToGrayScale() {
	        ...
	        for(int y = 0; y < image_pixels.getHeight(); y++) {
	        	for(int x = 0; x < image_pixels.getWidth();  x++) {
	        		Pixel p = getPixel(x,y,image_pixels);
	        		int redValue = p.getRedValue();
	        		int greenValue = p.getGreenValue();
	        		int blueValue = p.getBlueValue();
	        		int avr = ( redValue + greenValue + blueValue ) / 3;
	        		p.setRGB(avr, avr, avr);
	        	}
	        }
    ...
    public void StepColors() {
        int percentage = 10;
	        for(int y = 0; y < image_pixels.getHeight(); y++) {
	        	for(int x = 0; x < image_pixels.getWidth();  x++) {
	        		Pixel p = getPixel(x,y,image_pixels);
	        		int redValue = p.getRedValue() / percentage;
	        		int greenValue = p.getGreenValue() / percentage;
	        		int blueValue = p.getBlueValue() / percentage;
	        		p.setRGB(p.getRedValue() - redValue,p.getGreenValue() - greenValue,p.getBlueValue() - blueValue);
	        	}
	        }
    ...
    public void EmbossImage() {        
		...
		//        int x1 = image_copy.getWidth();
		//        int y1 = image_copy.getHeight();
		//        
		//        for(int y = 0; y < image_pixels.getHeight(); y++) {
		//        	for(int x = 0; x < image_pixels.getWidth();  x++) {
		//        		Pixel p = getPixel(x,y,image_pixels);
		//        		int redValue =  getPixel(x1 + 1, y1 - 1,image_pixels).getRedValue() - p.getRedValue();
		//        		int greenValue = getPixel(x1 + 1, y1 - 1,image_pixels).getRedValue() - p.getGreenValue();
		//        		int blueValue = getPixel(x1 + 1, y1 - 1,image_pixels).getRedValue() - p.getBlueValue();
		//        		
		//        		redValue += 125;
		//        		greenValue += 125;
		//        		blueValue += 125;
